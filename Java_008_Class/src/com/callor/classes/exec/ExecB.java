...
public class ExecB {
	...
	public static void main(String[] args) {
		...
		/*
		 * ScoreDtop 클래스를 사용하여 scoreDto 객체를 선언하고
		 * 생성자 method(ScoreDto())를 사용하여 객체를 초기화
		 * 
		 * scoreDto 객체를 초기화하여
		 * 사용할 준비가 되면 인스턴스가 되었다 라고 표현한다.
		 * 
		 * scoreDto 객체를 초기화 하는 순간
		 * ScoreDto 클래스에서 선언한 변수들을
		...
		scoreDto.his = (int)(Math.random() * 50) + 51;
		scoreDto.mus = (int)(Math.random() * 50) + 51;
		scoreDto.art = (int)(Math.random() * 50) + 51;
		scoreDto.phy = (int)(Math.random() * 50) + 51;
		scoreDto.stNum = "0001";
		scoreDto.stName = "홍길동";
